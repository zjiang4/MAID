...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$root=(Get-Location).Path; $url='http://127.0.0.1:8080'; $listener=Get-NetTCPConnection -LocalPort 8080 -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1; if($listener){$existing=Get-CimInstance Win32_Process -Filter ('ProcessId=' + $listener.OwningProcess) -ErrorAction SilentlyContinue; if($existing.Name -eq 'python.exe' -and $existing.CommandLine -like '*multi_agent_dev.py*'){Write-Host '[MAID] Restarting the existing MAID server...'; Stop-Process -Id $listener.OwningProcess -Force; for($i=0;$i -lt 20 -and (Get-NetTCPConnection -LocalPort 8080 -State Listen -ErrorAction SilentlyContinue);$i++){Start-Sleep -Milliseconds 250}}else{Write-Host '[ERROR] Port 8080 is used by another application.' -ForegroundColor Red; exit 2}}; $log=Join-Path $root 'maid_server.log'; $err=Join-Path $root 'maid_server_error.log'; $pidFile=Join-Path $root 'maid_server.pid'; $process=Start-Process -FilePath 'python' -ArgumentList 'multi_agent_dev.py' -WorkingDirectory $root -RedirectStandardOutput $log -RedirectStandardError $err -WindowStyle Hidden -PassThru; Set-Content -LiteralPath $pidFile -Value $process.Id -Encoding ascii; for($i=0;$i -lt 120;$i++){try{$response=Invoke-WebRequest -Uri $url -UseBasicParsing -TimeoutSec 2 -ErrorAction Stop; if($response.StatusCode -eq 200){Start-Process 'http://127.0.0.1:8080'; exit 0}}catch{}; Start-Sleep -Seconds 1}; Write-Host '[ERROR] MAID did not become ready within 120 seconds.' -ForegroundColor Red; Write-Host 'Check maid_server_error.log for details.'; exit 1"
